...
To change the default user avatar image, override `wagtailadmin/images/default-user-avatar.png` by placing your custom avatar image in a  [`static file directory`](inv:django#howto/static-files/index) with higher precedence than `wagtail.admin` (see Django's [`INSTALLED_APPS`](inv:django:std:setting#INSTALLED_APPS) for ordering).
